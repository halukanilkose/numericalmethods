...
analizDataFrame               = pandas.read_excel("analizMatrisi.xlsx")

t=0
while t<len(analizDataFrame):
    row = analizDataFrame.iloc[t]
    data = {
        "location": float(row['location']),
        "flightDuration": float(row['flightDuration']), 
        "timeStep": float(row['timeStep']),
        "materialProperties": {
            "thermalConductivity": float(row['thermalConductivity']),
            "density": float(row['density']),
            "specificHeat": float(row['specificHeat']),
            "emissivity": float(row['emissivity'])
        },
        "thickness": float(row['thickness']),
        "directionStep": float(row['directionStep']),
        "initialCondition": float(row['initialCondition'])
    }
    ...
    data.to_excel("{}_results.xlsx".format(t),index=None,header= header)
    t+=1
